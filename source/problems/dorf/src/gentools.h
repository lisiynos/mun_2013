...
#endif
char gen_file [MaxGenBuf];
int cur_test, test_lo, test_hi, num_tests;


void safe_snprintf (char * dest, size_t lim, char const * fmt, ...)
{
 va_list arg_list;

 va_start (arg_list, fmt);
 int chk = vsnprintf (dest, lim, fmt, arg_list);
 assert (chk < (int) lim);
 va_end (arg_list);
}

void safe_vsnprintf (char * dest, size_t lim, char const * fmt,
                     va_list arg_list)
{
 int chk = vsnprintf (dest, lim, fmt, arg_list);
 assert (chk < (int) lim);
...
void get_time_str (char * buf)
{
 ...
 safe_snprintf (buf, MaxGenBuf, "%04d-%02d-%02d %02d:%02d:%02d",
...
void init_gen (int argc, char * argv [])
{
 ...
 if (argc >= 3)
 {
  ...
  safe_snprintf (buf, MaxGenBuf, "tests %d..%d", test_lo, test_hi);
 }
 else if (argc >= 2)
 {
  if (sscanf (argv[1], "%d", &test_lo) != 1)
   assert (false);
  test_hi = test_lo;
  safe_snprintf (buf, MaxGenBuf, "test %d", test_lo);
 }
 else
 {
  test_lo = INT_MIN;
  test_hi = INT_MAX;
  safe_snprintf (buf, MaxGenBuf, "all tests");
 }
                          ...
                          const char * comment, ...)
{
 ...
 safe_vsnprintf (temp, MaxGenBuf, comment, arg_list);
 va_end (arg_list);

 safe_snprintf (cur_test_str, MaxGenBuf, TEST_STR, cur_test);
#ifdef GENERATE_ANSWERS
 safe_snprintf (cur_answer_str, MaxGenBuf, TEST_STR ANSWER_SUFFIX, cur_test);
#endif
 fprintf (flog, "Test %s (%s):", cur_test_str, temp);
 safe_snprintf (gen_file, MaxGenBuf, "%s", file_auto);
 if (strcmp (file_auto, "gen.cpp") != 0)
 {
  fprintf (flog, " file `%s',", file_auto);
 }
 fprintf (flog, " lines %d..", line_auto);

 gentools_random_internal::save_state (global_state);

 safe_snprintf (buf, MaxGenBuf, "%s: %s", GEN_PROBLEM_NAME, temp);
...
void end_test_internal (const int line_auto, const char * file_auto)
{
 ...
 if (strcmp (file_auto, gen_file) != 0)
 {
  fprintf (flog, " of file `%s'", file_auto);
 }
